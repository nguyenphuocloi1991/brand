...
async function updateViewCount (req, res) {
	try {
		await pool.query(mybatisMapper.getStatement('common', 'update_view_count', {concept_room_seq: req.body.concept_room_seq}))
...
async function updateDownloadImageCount (req, res) {
	try {
		await pool.query(mybatisMapper.getStatement('common', 'update_download_image_count', {concept_room_seq: req.body.concept_room_seq}))
		res.send({success: true})
	} catch (error) {
		res.send({success: false, message: error.message})
	}
